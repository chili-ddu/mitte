...
/* 떠 있는 것 전부 내리기 (2026-09-22 사용자: 상단 메뉴를 누르면 모든 모달이 닫혀야): 시트·켈라·고르기 모드·상세·확인·도움말 절·시즌 넘기기·드롭다운. 설정(톱니) 은 예외 — 상세 위에 뜬다 */
export function closeOverlays() { S.sheet = null; S.helpSec = null; S.cellsOpen = false; S.bedPick = null; S.palusMode = false; S.cellPop = null; S.cellSide = null; S.detail = null; S.shownDetail = null; S.seasonConfirm = false; S.shownSeason = false; S.ddOpen = null; }
...
function renderScreen() {
  ...
  { const town = renderTown(); app.append(sideToolsLand([{ icon: 'cells', title: '켈라', on: S.cellsOpen, onclick: () => { const was = S.cellsOpen; closeOverlays(); S.sheet = null; S.cellsOpen = !was; render(); } /* 켈라를 닫으면 그 안에서 연 검투사 상세도 같이 닫는다 */ }, { key: 'facilities', icon: 'facilities', title: '시설 강화' }, { key: 'doctors', icon: 'doctors', title: '독토르', badge: S.st.roster.filter(g => g.status === 'doctor').length }, { key: 'rivals', icon: 'rivals', title: '파밀리아' }, ...(S.view === 'medic' ? [{ key: 'medic' as const, icon: 'place' as const, title: '의무실' }] : [])])); app.append(town); /* 장소 서판: 그 장소의 안내·버튼(상인 다시 부르기·훈련 추천 배치·침상). 2026-09-22 사용자: 시트가 있는데 여는 길이 없었다 */
