...
export function drawLegendBurst(ctx: CanvasRenderingContext2D, a: BurstArgs) {
  ...
  switch (a.legend) {
    ...
    case 'spiculus': { // 황제의 방패: 황금 반구가 솟고 테두리를 빛이 돈다
      ...
      break; }
    case 'celadus': { // 함성: 흰 충격파 세 겹과 방사선, 관중석까지 번지는 빛
      ...
      break; }
    case 'crescens': { // 의사의 손: 푸른 빛의 그물 소용돌이와 물방울
      ctx.lineWidth = 1.2; for (let i = 0; i < 3; i++) { ctx.globalAlpha = 0.75 * fade; ctx.strokeStyle = i ? '#9fd8ff' : '#e6f6ff'; ctx.beginPath(); for (let s = 0; s <= 60; s++) { const q = s / 60; const ang = q * Math.PI * 6 + t * 4 + i * 2.1; const rr = 8 + q * 64; const px = x + Math.cos(ang) * rr, py = y - 6 + Math.sin(ang) * rr * 0.5 - q * 30; if (s === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py); } ctx.stroke(); }
      ctx.strokeStyle = '#cfefff'; ctx.globalAlpha = 0.5 * fade; for (let i = 0; i < 10; i++) { const ang = (i / 10) * Math.PI * 2 + t * 2; const r0 = 20, r1 = 62; ctx.beginPath(); ctx.moveTo(x + Math.cos(ang) * r0, y - 6 + Math.sin(ang) * r0 * 0.5); ctx.lineTo(x + Math.cos(ang) * r1, y - 6 + Math.sin(ang) * r1 * 0.5 - 20); ctx.stroke(); }
      for (let i = 0; i < 30; i++) { const h0 = hash01(seed + i, 8), h1 = hash01(seed + i, 9); const life = (k * 1.4 + h1) % 1; const px = x + (h0 - 0.5) * 120, py = y + 30 - life * 110; ctx.globalAlpha = (1 - life) * 0.9 * fade; ctx.fillStyle = '#bfe9ff'; ctx.beginPath(); ctx.ellipse(px, py, 1.6, 3.2, 0, 0, Math.PI * 2); ctx.fill(); }
      break; }
    case 'priscus': { // 버티는 자: 땅이 갈라지고 돌기둥이 솟는다
    ...
    case 'verus': { // 진실의 창: 앞으로 뻗는 빛의 창과 흩어지는 불꽃
      ...
      break; }
    case 'tetraites': { // 유리잔의 기수: 흙먼지 폭풍과 흩날리는 유리 조각
      ...
      for (let i = 0; i < 36; i++) { const h0 = hash01(seed + i, 17), h1 = hash01(seed + i, 18), h2 = hash01(seed + i, 19); const q = Math.min(1, k * 1.8); const ang = h0 * Math.PI * 2; const d = 10 + q * (40 + h1 * 90); const px = x + Math.cos(ang) * d, py = y - 6 + Math.sin(ang) * d * 0.6 + q * q * 60 * h2; ctx.save(); ctx.translate(px, py); ctx.rotate(t * 8 + i); ctx.globalAlpha = (1 - q * 0.6) * fade; ctx.fillStyle = i % 4 ? 'rgba(200,240,255,0.85)' : '#ffffff'; ctx.beginPath(); ctx.moveTo(0, -4 - h1 * 4); ctx.lineTo(3 + h2 * 3, 0); ctx.lineTo(0, 4 + h0 * 4); ctx.lineTo(-3 - h1 * 3, 0); ctx.closePath(); ctx.fill(); ctx.strokeStyle = '#7fc7e8'; ctx.lineWidth = 0.6; ctx.stroke(); ctx.restore(); }
      break; }
    case 'hermes': { // 세 가지 무기: 세 빛깔의 잔상이 겹친다
      ...
      break; }
    case 'columbus': { // 비둘기: 흰 깃털이 소용돌이치며 내려앉는다
      for (let i = 0; i < 40; i++) { const h0 = hash01(seed + i, 20), h1 = hash01(seed + i, 21); const q = (k * 1.2 + h1) % 1; const ang = h0 * Math.PI * 2 + q * 3 + t * 0.5; const d = 16 + q * 90; const px = x + Math.cos(ang) * d, py = y - 40 - h0 * 30 + q * 110 + Math.sin(t * 6 + i) * 4; ctx.save(); ctx.translate(px, py); ctx.rotate(Math.sin(t * 4 + i) * 0.8 + ang); ctx.globalAlpha = (1 - q * 0.7) * fade; ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.ellipse(0, 0, 6, 2.4, 0, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#cfcfcf'; ctx.lineWidth = 0.6; ctx.beginPath(); ctx.moveTo(-6, 0); ctx.lineTo(6, 0); ctx.stroke(); ctx.restore(); }
