...
function renderHelp(): Node {
  ...
  const all = h('div', { class: 'help' },
    ...
    sec('미시오 (패자의 목숨)',
      ...
      row('결과', `살아남으면 ${Math.round(M.injuryChance * 100)}% 확률로 부상 (${injurySeasons(S.st)}시즌 출전 불가 — 침상에 누우면 시즌마다 1 낫고 ${bedCostOf(S.st)} HS, 침상 밖이면 ${Math.round(CONFIG.injury.natural.heal * 100)}% 낫거나 ${Math.round(CONFIG.injury.natural.worsen * 100)}% 덧나며 ${CONFIG.injury.deathAt}에 이르면 죽는다). 실패하면 사망하고 주최자가 배상 (구매가×${CONFIG.deathComp.priceMult} + 승수×${CONFIG.deathComp.perWin}).`)),
    sec('주최자 (에디토르)',
