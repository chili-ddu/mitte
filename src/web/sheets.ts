...
import { rivalDef, rivalTraits, RIVAL_TRAIT_KO, RIVAL_TRAIT_DESC } from '../core/rivals.js';
      ...
      h('div', { class: 'rivals' }, ...S.st.rivals.map(rv => { const star = rivalStar(rv); const inContracts = S.st.contracts.filter(c => c.rivalId === rv.id).length;
        ...
        return h('div', { class: 'rivalcard' },
          h('div', { class: 'rvinfo' }, /* 왼쪽: 파밀리아 정보 (2026-09-17 사용자) */
            h('div', { class: 'rvname' }, h('span', { class: 'sq small', style: `background:${teamColorOf(rv.color).ink}`, title: `${teamColorOf(rv.color).ko}색` }), ' ', h('b', {}, rv.name), h('span', { class: `badge prof ${rv.profile ?? 'local'}`, title: rivalDef(rv)?.desc ?? '' }, rv.profile === 'grand' ? '최대 루두스' : rv.profile === 'major' ? '큰 루두스' : '지방 파밀리아'), ...rivalTraits(rv).map(t => h('span', { class: `badge chip rvtrait ${t}`, title: RIVAL_TRAIT_DESC[t] }, RIVAL_TRAIT_KO[t])), rivalDef(rv)?.city ? h('span', { class: 'hint' }, `${rivalDef(rv)!.city} · 제${rivalDef(rv)!.stage}막`) : null, inContracts ? h('span', { class: 'badge revenge' }, `이번 시즌 계약 ${inContracts}`) : null), /* 특징 칩·도시 (2026-09-22) */
            ...
            S.phase === 'manage' ? (() => { const why = canStarBet(S.st, rv); const mine = S.st.contracts.find(c => c.starBet && c.rivalId === rv.id); const def = rivalDef(rv); return h('div', { class: 'meta' }, mine ? h('span', { class: 'hint' }, `간판 내기를 걸었다 — ${mine.venue}. 편성에서 세운 검투사가 판돈이다`) : h('button', { class: `small${starBetRival(S.st, rv) ? ' primary' : ''}`, disabled: !!why, title: why ?? `우리 검투사를 걸고 간판 ${rivalStar(rv)?.name ?? ''}과(와) 1대1. 이기면 제${def?.stage ?? S.st.stage}막 졸업, 간판을 데려오거나 값을 받는다. 지면 세운 검투사를 넘긴다. 처형은 없다`, onclick: () => { const c = sendStarBet(S.st, rv); if (c) { sfx.drum(1); toast(`${rv.name}에 간판 내기를 걸었다 — 편성에서 세운다`, 'good'); save(); render(); } } }, `간판 내기${def ? ` · 제${def.stage}막` : ''}`)); })() : null, /* 간판 내기 (2026-09-22 사용자): 판돈은 검투사, 이기면 막 졸업 */
...
function renderHelp(): Node {
  ...
  const all = h('div', { class: 'help' },
    ...
    sec('성장 (초기 굴림 없음 — 현재치는 유형 × 서열, 개체 차이는 잠재치·나이·성장형·자질)',
      ...
      row('상한', '숫자는 감춰져 있다. 칸 막대 위 ▲ 가 그 능력치의 상한 자리, 닿으면 카드에서 값이 굵어진다. 넷 다 닿은 검투사는 더 자라지 않는다 — 팔거나 독토르로'),
