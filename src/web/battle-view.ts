...
type CrowdMood = 'calm' | 'cheer' | 'gasp' | 'hush' | 'missio' | 'frenzy';
function drawCheerOverlay(ctx: CanvasRenderingContext2D, seats: ReturnType<typeof seatList>, t: number, cheer: number, view: { x0: number; y0: number; x1: number; y1: number }, cloth = 0, surge = 0) {
  ...
  for (const sd of seats) {
    ...
    if (sd.x < view.x0 - 60 || sd.x > view.x1 + 60 || sd.y < view.y0 - 100 || sd.y > view.y1 + 30) continue;
    const local = Math.max(0, surge - hash01(sd.j * 7 + 2, sd.k * 11 + 4) * 0.42);
    const up = cheer > 0 || local > 0;
    const bob = up ? Math.abs(Math.sin(t * (16 + local * 7) + sd.j * 0.7 + sd.k)) * (8 + local * 7) * Math.min(1, cheer * 2 + local) : 0;
...
function drawRailSpectators(ctx: CanvasRenderingContext2D, t: number, view: { x0: number; y0: number; x1: number; y1: number }, mood: CrowdMood, force: number, teamColor: string) {
  const ry = floorRy(1), baseY = ry * 1.18;
  const people = [
    { x: -WORLD.rx * 0.58, y: baseY, sc: .78, kind: 'lanista' as const, seed: 2, f: 1 as const },
    { x: -WORLD.rx * 0.48, y: baseY + 4, sc: .68, kind: 'citizen' as const, seed: 5, f: 1 as const },
    { x: WORLD.rx * 0.48, y: baseY + 5, sc: .68, kind: 'citizen' as const, seed: 9, f: -1 as const },
    { x: WORLD.rx * 0.58, y: baseY, sc: .7, kind: 'citizen' as const, seed: 13, f: -1 as const },
  ];
  const active = mood !== 'calm' || force > 0.05;
  ctx.save(); ctx.strokeStyle = '#8f7047'; ctx.lineWidth = 3; ctx.globalAlpha = 0.65; ctx.beginPath(); ctx.moveTo(Math.max(view.x0, -WORLD.rx * .72), baseY - 18); ctx.lineTo(Math.min(view.x1, WORLD.rx * .72), baseY - 18); ctx.stroke(); ctx.restore();
  for (const p of people) {
    if (p.x < view.x0 - 40 || p.x > view.x1 + 40 || p.y < view.y0 - 80 || p.y > view.y1 + 80) continue;
    const wave = Math.sin(t * (active ? 7 : 2.4) + p.seed) * (active ? 1 : .35), jump = Math.max(0, Math.sin(t * 9 + p.seed)) * (mood === 'frenzy' ? 7 : mood === 'cheer' ? 4 : 0) * Math.min(1, force + .4);
    let sk: Skeleton = p.kind === 'lanista' ? NPC_POSES.watch : NPC_POSES.stand;
    if (mood === 'hush') sk = p.kind === 'lanista' ? NPC_POSES.watch : NPC_POSES.tiptoe;
    else if (mood === 'gasp') sk = NPC_POSES.point;
    else if (mood === 'missio') sk = p.seed % 2 ? NPC_POSES.point : NPC_POSES.tiptoe;
    else if (mood === 'cheer' || mood === 'frenzy') sk = { ...NPC_POSES.point, frontArm: [-155 + wave * 8, -8], backArm: [-128 - wave * 8, 16], headBob: -2 - jump * .2, lift: jump };
    else sk = { ...sk, headBob: sk.headBob + wave };
    drawStickman(ctx, 'murmillo', { x: p.x, y: p.y - jump, scale: p.sc, facing: p.f, skeleton: sk, t: t + p.seed, ink: INK, bare: true, garment: p.kind === 'lanista' ? 'toga' : 'tunic', garmentColor: p.kind === 'lanista' ? '#f3ead0' : (p.seed % 2 ? '#c8a878' : '#b9c2a8'), garmentStripe: p.kind === 'lanista' ? teamColor : undefined, beard: p.kind === 'lanista' });
  }
}
...
export function renderBattle() {
  const r = S.report!; const foe = rivalInk(r.contract.rivalId); const foeKo = r.contract.rivalId !== undefined ? (teamColorOf(S.st.rivals.find(x => x.id === r.contract.rivalId)?.color).ko + '색') : '자주색'; /* 상대 파밀리아의 안료 (2026-09-21) */
  ...
  const wrap = h('div', { class: 'panel battle' }, // 헤더 아래 장면 영역을 채운다 (헤더는 그대로). 편성(VS) 블록은 경기장 위에 겹쳐 띄웠다가 잠시 뒤 위로 접힌다
    h('div', { class: 'stage' }, canvas, lineup, lineupTab,
      legendShown ? null : h('div', { class: 'legend' }, h('span', { style: `color:${myInk()};font-weight:700` }, `■ ${teamColorOf(S.st.color).ko}색 방패·허리천 = 내 루두스`), '   ', h('span', { style: `color:${foe};font-weight:700` }, `■ ${foeKo} = 상대 파밀리아`)),
...
// 경기장 높이 = 남는 높이 (스크롤 없이 바 바로 위까지)
  ...
  const addWall = (text: string, red = false) => { if (wallMarks.length > 12) wallMarks.shift();
    const a = -Math.PI * (0.12 + 0.76 * ((wallMarks.length * 0.37 + hash01(wallMarks.length, 3)) % 1)); wallMarks.push({ a, text, t: 0, red }); };
  const sandMarks: { x: number; y: number; kind: 'step' | 'blood' | 'drag'; a: number; r: number; seed: number }[] = []; // 모래에 남는 자취 (경기 내내 쌓인다)
  const addMark = (x: number, y: number, kind: 'step' | 'blood' | 'drag', r = 6) => { if (sandMarks.length > 160) sandMarks.shift(); sandMarks.push({ x, y, kind, a: kind === 'blood' ? 0.3 : 0.24, r, seed: sandMarks.length }); };
  const fx: { kind: FxKind; x: number; y: number; t: number; dir: number; seed: number; id?: number; to?: number; life?: number; x2?: number; y2?: number; main?: string }[] = []; // id: 붙어 다닐 검투사 · to: 상대 · life: 총 시간
  ...
  let crowdCheer = 0, crowdSurge = 0, hushUntil = -1; // hushUntil: 판정 직후의 정적 (소리도 함성도 멎는다)
  let crowdMood: CrowdMood = 'calm', crowdMoodUntil = -1;
  const rouse = (mood: CrowdMood, force: number, dur = 1.1) => { crowdMood = mood; crowdMoodUntil = Math.max(crowdMoodUntil, ct + dur); crowdSurge = Math.max(crowdSurge, force); };
  const shout = (text: string, x: number) => { shouts.length = 0; shouts.push({ text, t: 1.2, x }); crowdCheer = 0.7; rouse('cheer', 0.65, 1.2); sfx.cheer(0.5); };
  ...
  const mountedHoldUntil: Record<number, number> = {}, mountedStrikeUntil: Record<number, number> = {}; // 기마 찌르기: 타격 순간까지 말 위 연출을 붙잡는다
  ...
  function fireEvents(ct: number) {
    ...
    while (ei < r.events.length && r.events[ei].t <= ct) {
      ...
      if (e.dictata && e.dictata !== 'net' && e.dictata !== 'lasso') flash.push({ id: aid, t: 1.3, text: DICTATA_NAME(e.dictata), color: '#c58a1a' }); if (e.dictata === 'dismount') { slowUntil = Math.max(slowUntil, ct + 0.5); crowdCheer = Math.max(crowdCheer, 0.9); rouse('cheer', 0.9, 1.2); } /* 기마 찌르기: 첫 충돌은 볼거리 */ /* 타격에 실린 딕타타 이름 — 플레이어는 여기서 유형의 동작을 배운다 */
      ...
      pending.push({ at: ct + hitDelay + (e.net ? 0.55 : 0), fn: () => {
        ...
        { const nl = shareOf(sideIds.A) - shareOf(sideIds.B); // 체력 우세가 뒤집히는 순간: 잠깐 늦추고 관중이 술렁인다
          if (flips < MAX_FLIPS && nl > 0.02 && worstA <= -REVERSE_GAP) { flips++; worstA = 0; slowUntil = Math.max(slowUntil, ct + 0.55); crowdCheer = 1; rouse('cheer', 1, 1.5); sfx.cheer(0.8); const p = posAt(ct)[aid]; shouts.length = 0; shouts.push({ text: '뒤집혔다!', t: 1.6, x: p.x }); flash.push({ id: aid, t: 1.6, text: '반전!', color: '#9b1f14' }); }
          else if (flips < MAX_FLIPS && nl < -0.02 && worstB <= -REVERSE_GAP) { flips++; worstB = 0; slowUntil = Math.max(slowUntil, ct + 0.55); crowdCheer = 0.6; rouse('gasp', 0.8, 1.3); sfx.boo(); const p = posAt(ct)[aid]; shouts.length = 0; shouts.push({ text: '뒤집혔다!', t: 1.6, x: p.x }); }
        ...
        if (!e.downed && (e.targetHp ?? 0) > 0 && (e.targetHp ?? 0) / r.initialHp[tid] <= 0.08 && (hp[aid] ?? 0) / r.initialHp[aid] >= 0.4 && !brink.has(tid)) { brink.add(tid); /* 상대는 멀쩡한데 벼랑 끝에서 버텼다 */ // 벼랑 끝에서 버텼다
          slowUntil = Math.max(slowUntil, ct + 0.35); const p = posAt(ct)[tid]; shouts.length = 0; shouts.push({ text: '아직 섰다!', t: 1.4, x: p.x }); crowdCheer = Math.max(crowdCheer, 0.9); rouse('cheer', 0.85, 1.2); }
        ...
        if (e.crit) { flash.push({ id: tid, t: 1.3, text: '치명타!', color: '#9b1f14' }); addWall('HABET', true); rouse('gasp', 1, 1.3); }
  ...
  function draw(ct: number, dt: number) {
    ...
    if (wallMarks.length && tilt > 0.75) { ctx.save(); // 포디움 벽에 긁힌 낙서 (뒤쪽 벽면)
      const rxw = WORLD.rx, ryw = floorRy(tilt);
      for (const m of wallMarks) { m.t = Math.min(1, m.t + dt * 2.5);
        const wx = Math.cos(m.a) * rxw * 0.94, wy = Math.sin(m.a) * ryw * 0.94 - 6;
        ctx.save(); ctx.translate(wx, wy); ctx.rotate(-0.06 + hash01(Math.round(m.a * 50), 7) * 0.12);
        ctx.globalAlpha = 0.5 * m.t; ctx.fillStyle = m.red ? '#9b2c1c' : '#5a4224'; ctx.font = 'bold 8px serif'; ctx.textAlign = 'center';
        ctx.fillText(m.text, 0, 0); ctx.restore(); }
      ctx.restore(); }
    if (sandMarks.length && tilt > 0.6) { ctx.save(); // 모래의 기억: 발자국·끌린 자국·굳은 핏자국이 경기 내내 쌓인다
      for (const m of sandMarks) { ctx.globalAlpha = m.a * (m.kind === 'blood' ? 0.8 : 0.5);
        ctx.fillStyle = m.kind === 'blood' ? '#6e1f12' : '#b9a26f';
        ctx.beginPath(); ctx.ellipse(m.x, m.y, m.r, m.r * (m.kind === 'drag' ? 0.16 : 0.3), m.kind === 'drag' ? 0.1 : 0, 0, Math.PI * 2); ctx.fill(); }
      ctx.restore(); }
    if (crowdMoodUntil < ct && crowdMood !== 'calm') crowdMood = 'calm';
    const liveMood: CrowdMood = frenzy ? 'frenzy' : judge && judge.stage <= 2 ? (hushUntil > 0 || judge.stage === 1 ? 'hush' : 'missio') : crowdMood;
    if (tilt >= 0.5) { drawCheerOverlay(ctx, seats, ct, crowdCheer, view, crowdCloth, crowdSurge); drawRailSpectators(ctx, ct, view, liveMood, Math.max(crowdCheer > 0 ? 0.45 : 0, crowdSurge), myInk()); }
    if (crowdCheer > 0 && !frenzy) crowdCheer -= dt;
    if (crowdSurge > 0) crowdSurge = Math.max(0, crowdSurge - dt * 0.75);
    // 주최자 반응 (세레모니)
    if (hostMood !== 'none') {
    ...
    for (const u of order) {
      ...
      const mountedNow = ct < Math.max(r.mounted[id] ?? -1, mountedHoldUntil[id] ?? -1); // 에퀘스: 첫 충돌 전후까지 말 위 자세를 따로 쓴다
      if (mountedNow && isAlive) sk = mountedSkeleton(ct + id, ct < (mountedStrikeUntil[id] ?? -1)); // 걸음/공격 자세를 쓰면 말 위에서 함께 걷는 것처럼 보인다
      const winded = isAlive && !mountedNow && ((p as { sta?: number }).sta ?? 100) < CONFIG.stamina.windedAt; // 숨이 찬 동안: 어깨를 들썩이며 몸이 조금 내려앉는다
      ...
      if (a.clip.startsWith('combo') && busy && el > 120 && el < 300) { // 연속 공격: 2타의 잔상 (60ms 전 자세를 흐리게 겹쳐 그린다)
        ...
        drawStickman(ctx, u.g.type, { x: p.x + recoilDx + jx + lapDx + exitDx - face[id] * 6, y: p.y + 30 * SC + jy, scale: 1.15 * SC, facing: face[id], skeleton: gs, t: ct, team: u.side === 'A' ? myInk() : foe, accessories: accessoriesOf(u.g) }); ctx.restore(); }
      if (mountedNow) drawHorse(ctx, p.x + recoilDx + jx + lapDx + exitDx, p.y + 30 * SC + jy, 1.15 * SC, face[id], ct, u.side === 'A' ? myInk() : foe); /* 에퀘스: 첫 돌진까지 말 위 (2026-09-18) */
      drawStickman(ctx, u.g.type, { x: p.x + recoilDx + jx + lapDx + exitDx - (mountedNow ? face[id] * 4 * SC : 0), y: p.y + 30 * SC + jy + breath - (mountedNow ? 31 * SC : 0), scale: 1.15 * SC, facing: face[id], skeleton: sk, t: ct, wobble: (isBound || winded) && !busy, noNet: !!netAway[id], team: u.side === 'A' ? myInk() : foe, accessories: accessoriesOf(u.g) });
      ...
      ctx.fillStyle = u.side === 'A' ? myInk() : foe; ctx.font = 'bold 11px sans-serif'; ctx.textAlign = 'center';
    ...
    for (let k = fx.length - 1; k >= 0; k--) {
      ...
      if (f.kind === 'tell') { const life = f.life ?? 0.2, k2 = 1 - f.t / life; ctx.strokeStyle = '#9b2c1c'; ctx.globalAlpha = 0.18 + k2 * 0.32; ctx.lineWidth = 1.8 + k2 * 1.2; ctx.beginPath(); ctx.arc(f.x - f.dir * 10, f.y, 18 + k2 * 8, -1.15 * f.dir + (f.dir > 0 ? 0 : Math.PI), 0.45 * f.dir + (f.dir > 0 ? 0 : Math.PI), f.dir < 0); ctx.stroke(); }
        ...
        if (f.kind === 'ghost' && u && cur) { ctx.globalAlpha = 0.35 * (1 - k2); drawStickman(ctx, u.g.type, { x: cur.x - f.dir * (10 + k2 * 26), y: cur.y + 30 * SC, scale: 1.15 * SC, facing: f.dir as 1 | -1, pose: 'guard', t: 0, team: u.side === 'A' ? myInk() : foe, accessories: accessoriesOf(u.g) }); }
        ...
        else if (f.kind === 'weaponfly' || f.kind === 'weapondown') { // 날아가는 무기와 모래에 떨어진 무기
          const fly = f.kind === 'weaponfly', k3 = fly ? k2 : 1;
          const wx = fly ? (f.x2 ?? f.x) + (f.x - (f.x2 ?? f.x)) * k3 : f.x, wy = fly ? (f.y2 ?? f.y) + (f.y - (f.y2 ?? f.y)) * k3 - Math.sin(k3 * Math.PI) * 40 : f.y;
          ctx.save(); ctx.translate(wx, wy); ctx.rotate(fly ? k3 * 9 * f.dir : 1.5); ctx.globalAlpha = fly ? 1 : 0.85;
          ctx.strokeStyle = '#6b4a22'; ctx.lineWidth = fly ? 3 : 2.4; ctx.lineCap = 'round'; ctx.beginPath();
          if (f.main === 'trident') { ctx.moveTo(-16, 0); ctx.lineTo(12, 0); ctx.moveTo(12, -6); ctx.lineTo(18, -6); ctx.moveTo(12, 0); ctx.lineTo(19, 0); ctx.moveTo(12, 6); ctx.lineTo(18, 6); }
          else if (f.main === 'spear') { ctx.moveTo(-20, 0); ctx.lineTo(18, 0); ctx.moveTo(12, -4); ctx.lineTo(19, 0); ctx.lineTo(12, 4); }
          else if (f.main === 'sica') { ctx.moveTo(-8, 4); ctx.quadraticCurveTo(4, 2, 12, -7); }
          else { ctx.moveTo(-9, 0); ctx.lineTo(11, 0); ctx.moveTo(-9, -3); ctx.lineTo(-9, 3); }
          ctx.stroke(); ctx.restore(); }
        else if (f.kind === 'sandwall') { // 마지막 일격의 먼지 장막: 확 일었다가 가라앉으며 승자만 남는다
          const up = Math.min(1, k2 * 3.2), fade = 1 - Math.max(0, (k2 - 0.35) / 0.65);
          ctx.globalAlpha = 0.85 * fade; ctx.fillStyle = '#d9c49a';
          for (let i = 0; i < 26; i++) { const a = (i / 26) * Math.PI * 2 + f.seed; const rr = (18 + hash01(f.seed + i, 5) * 90) * up; const rx = 26 + hash01(f.seed + i, 9) * 40;
            ctx.beginPath(); ctx.ellipse(f.x + Math.cos(a) * rr, f.y + 18 + Math.sin(a) * rr * 0.4, rx * (0.5 + up * 0.8), rx * 0.4 * (0.5 + up * 0.8), 0, 0, Math.PI * 2); ctx.fill(); }
          ctx.globalAlpha = 1; }
  ...
  canvas.onpointerdown = () => { if (!judge || done || judge.stage > 2 || hushUntil > 0) return; crowdCloth = Math.min(1, crowdCloth + 0.15); crowdCheer = 0.7; rouse('missio', 0.85, 1.2); sfx.chant(1); shouts.length = 0; shouts.push({ text: '미테!  미테!', t: 1.0, x: (Math.random() - 0.5) * 500 }); };
  ...
  const anim = () => {
    ...
    if (intro < INTRO_ZOOM) { intro += Math.min(0.3, realRaw); dt = 0; // 입장 동안은 경기를 멈춰 둔다
      ...
      if (st2 !== introStage) { introStage = st2; // 뚜둥 — 한쪽씩 소개하고 마지막에 가운데로
        if (st2 === 1) { introText.big = S.st.lanista.name + ' 의 파밀리아'; introText.sub = r.team.map(g => g.name).join(' · '); introText.t = 1.5; sfx.drum(1); crowdCheer = Math.max(crowdCheer, 0.8); rouse('cheer', 0.7, 1.2); }
        else if (st2 === 2) { introText.big = rivalName; introText.sub = r.contract.enemy.map(g => g.name.replace('(적)', '')).join(' · '); introText.t = 1.5; sfx.drum(1); crowdCheer = Math.max(crowdCheer, 0.8); rouse('cheer', 0.65, 1.2); }
        else if (st2 === 3) { introText.big = ''; introText.sub = ''; rouse('cheer', 0.9, 1.0); sfx.cheer(0.9); } }
    ...
    if (!done && !ceremonyStarted && r.winner !== 'draw' && ct >= r.duration + 1.0 + (hasJudge ? JUDGE : 0)) {
      ...
      crowdCheer = 999; crowdSurge = 1; crowdMood = 'frenzy'; crowdMoodUntil = Infinity; frenzy = true; sfx.fanfare(); sfx.cheer(1); // 결과 보기까지 관중 전원 열광
...
function turningPoint(r: FightReport): string | null {
  ...
  return null; /* 몸 상태 한 줄은 2026-09-22 뺐다 */
}
...
function mountedSkeleton(t: number, thrust = false): Skeleton {
  const bob = Math.sin(t * 14) * 2.2;
  if (thrust) return { lean: 24, frontArm: [105, -10], backArm: [-8, 44], frontLeg: [82, -94], backLeg: [45, -76], headBob: -2, sink: 14, lift: Math.max(0, bob) * 0.5 };
