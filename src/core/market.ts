...
export function offerMarket(rng: Rng, season = 1): Gladiator[] {
  ...
  if (rng.chance(0.5)) list.push(withOrigin(rng, makeGladiator(rng, 'veteranus'), season));
  return list;
}
...
export function offerApplicants(rng: Rng, season: number, fame: number): Gladiator[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (g.rank === 'veteranus') { g.wins = Math.max(g.wins, 3); g.fights = Math.max(g.fights, 5); }
    out.push(g);
  }
  return out;
}
