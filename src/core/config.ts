...
export const CONFIG = {
  ...
  contractDiff: { lateFrom: 12, lateWeakToStrong: 0.5 }, // 약·중·강은 이제 내 전력 비율이 아니라 그 파밀리아 명단 안의 아래·가운데·위 셋째 (2026-09-22 사용자: 상대는 세상의 것 — 약한 파밀리아부터, 명성이 오르면 센 파밀리아가 나타난다) // 2026-09-21: .85/1.1/1.4 → .75/.95/1.25 — 흥행 계약은 '지지 않을 만한 경기'(도전 계약이 위험을 맡는다). 봇 흥행 승률 47 → 54% // 계약 상대 강도(내 최선 팀 전력 대비) · 후반(lateFrom 시즌부터) 약한 계약이 강한 계약으로 바뀔 확률
  /* 2026-09-22: 계약 상대를 내 전력에 맞춰 뽑던 정규화를 없앴다 — 등급 1 은 지방 파밀리아, 2 는 큰 루두스, 3 은 최대 루두스의 실제 명단에서 온다. 파밀리아 품질은 rivals.quality */
  ...
  prizePerTier: 1500,
  postersMax: 4,
  foreign: { chance: 0.15, prize: 1.6, gift: true }, // 타지 흥행(2026-09-22 사용자: 이벤트성으로): 시즌마다 15% 로 타지 라니스타의 검투사를 세운 계약 하나 — 상금 ×1.6, 이기면 쓰러진 상대 검투사를 선물로 (켈라가 차 있으면 값의 절반을 돈으로)
  activeRivalsMax: 2, // 한 시즌에 상대하는 파밀리아 수 (2026-09-22 사용자: 넷은 많다 → 셋 → 둘. 둘이면 "누구와 붙을까"가 되고 같은 집과 여러 시즌 붙어 라이벌이 쌓인다)
  starBet: { strongMul: 1.0 }, // 간판 내기(2026-09-22 사용자): 판돈은 출전 검투사. 이기면 막 졸업 + 간판을 데려오거나 그 값을 돈으로. 처형·즉사 없음(두 라니스타의 약속, 스폰시오 관습에 기댐) // 한 시즌에 상대하는 파밀리아 수 (2026-09-22 사용자: 넷은 많다, 셋을 골라 진행)   // 포룸 공고벽의 자리 수(2×2). 도전장을 받거나 걸어 계약이 이보다 많아지면 가장 값싼 보통 계약 하나를 벽에서 뗀다 (2026-09-22 사용자: 광고가 5개가 될 때가 있다)
  ...
  ludus: { // 루두스 시설 (장기 지출처). 모든 항목이 유한 단계
    cells: { start: 3, max: 12, per: 3, addCost: (n: number) => 8000 + n * 4000, qualityCost: [3000, 6000, 10000] }, // 칸 증축 = 한 줄(3칸)씩, 3 → 6 → 9 → 12 (2026-09-22 사용자: 15 → 12 — 특성을 편성에서 세게 된 뒤로 15칸의 이유가 사라졌다, 3×4 격자). 비용 8,000 + 4,000×n (n = 증축 횟수, 총 36,000) / 칸마다 숙소 질 0~3
  ...
  origins: { // 검투사 확보 경로 (고증: 노예 매매, 전쟁 포로, 형벌 담나티 아드 루둠, 자유민 아욱토라티)
    captive:    { price: 0.65, atk: 1, hp: 10, missio: -0.05 },  /* 공격 +2 → +1 (2026-09-22 눈금 절반) */   // 포로: 싸고 강하지만 관중이 이방인에게 냉담 (미시오 −5%)
  ...
  atkScale: 2, // 공격 눈금(2026-09-22 사용자): 표시·성장은 한 자릿수(기본 7~9), 전투는 여기를 곱해 옛 14~17 과 같은 피해. 훈련 상승분과 피로 벌점은 이것으로 나눈다
  power: { hp: 0.52, atk: 8.3, def: 3.2, spd: 1.2, hand: 2.5 }, // 측정 비례(공 9.0 자)에 ×0.92 — 평균 전력(≈182)과 값·등급 문턱이 전과 같게 (2026-09-22 stat:worth 재측정 뒤 평균 전력이 8.8% 올라 값이 따라 올랐다) // 2026-09-22 재측정(stat:worth 2000, 공격 눈금 절반 뒤): +1 당 승률 이득 hp 0.95(÷10) · 공 14.98 · 방 5.77 · 걸음 2.16 · 손놀림 4.47 %p → 공 9.0 자로 비례. 09-20 값(방 8.6·손 4.1)은 장비 규칙 삭제 전이라 방어가 과대, 손놀림이 과소였다
  ...
  typePower: { murmillo: 3.2, secutor: 0.9, thraex: -2.7, retiarius: -1.0, hoplomachus: 4.9, provocator: 5.7, eques: -2.3, dimachaerus: -5.8, scissor: -3.5, laquearius: -1.2 } /* 2026-09-22 유형 재분배 뒤 재적합: 같은 전력 짝 800판에서 승률이 50 에서 벗어난 만큼(1.7%p ≈ 전력 1) 더하고 뺐다 */ as Record<string, number>, // 유형 보정(2026-09-18 재측정): 장비 규칙을 뺀 전투에서 전력이 같은 짝의 1대1 승률이 50%가 되도록 맞춘 값(상성표 셀 0.445~0.555). 무거운 유형(HP·방어)이 전력식보다 세게 나와 +, 빠른 유형이 −. 유형 정리 뒤 다시 잰다
