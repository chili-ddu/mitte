...
export const CONFIG = {
  ...
  maxTurns: 30,
  net: { base: 0.80, perSpd: 0.06, min: 0.30, sec: 1.2 }, // 그물(2026-09-17): 던지면 반드시 묶이던 것을 상대 속도에 따라 빗나가게. 성공 확률 = base − 상대 속도×perSpd (최저 min) — 느린 무르밀로에겐 62%, 빠른 레티아리우스에겐 32%. 걸리면 sec 초. 빗나가면 그물을 잃는다 — 기록의 레티아리우스는 그물을 놓치면 삼지창만으로 싸웠다. 걸린 상대는 막지도 받아넘기지도 못하고 방어가 열린다(빈틈 강타와 같은 처리): 그물에 감긴 채로 방패를 들 수는 없다
  legs: { chance: 0.16, perGreave: 0.4, slow: 0.35, sec: 3 }, // 다리 노리기(2026-09-17): 근접이 칠 때 확률로 하체를 노린다. 정강이받이(오크레아) 하나마다 확률 ×(1−perGreave). 맞으면 sec 초 동안 걸음 −slow. 고증: 세쿠토르는 정강이받이도 투구도 없는 레티아리우스의 하체를 노렸다
  parry: { cut: 0.5, scale: 1 }, // 무기 받아넘기기(2026-09-17): 방패로 막지 못했을 때 자기 무기로 받아 피해 −cut. 확률은 주무기+보조무기의 parry 합 ×scale. 방패가 작거나 없는 무장의 몫 — 닳지 않지만 감소폭이 방패보다 작다
  polearm: { inside: 34, closePenalty: 0.72, poke: 0.5, pokeGap: 1.2, pokeCost: 0.8, push: 12 }, // 창·삼지창(2026-09-17): 사거리를 실제 이점으로. 밖에서 안으로 들어오는 순간 한 번 찌르고(피해 poke 배율, 같은 상대에겐 pokeGap 초에 한 번, 찌른 대가로 다음 공격이 pokeCost 만큼 늦다) 조금 밀어낸다(push). 대신 품 안(inside 보다 가까움)으로 파고들리면 창을 짧게 잡아 피해 ×closePenalty — 기록의 호플로마쿠스는 품에 들어오면 단검을 뽑아야 했다
  shove: { chance: 0.25, push: 15, stamina: 14 }, // 방패 밀어붙이기(2026-09-17): 큰 방패(스쿠툼·중형)는 공격 사이에 상대를 밀어 숨을 깎는다. 느린 대신 파고드는 무르밀로·세쿠토르의 몫
  shield: { cut: 0.55, wear: 0.04, min: 0.10, scaeva: 0.5 }, // 방패(2026-09-17): 첫 타 1회 고정 감소 → 매 타 확률 막기. 막으면 피해 −cut, 막을 때마다 그 방패의 확률이 wear 만큼 닳는다(최저 min) — 실제 방패는 받아내며 쪼개졌다. 왼손잡이(scaeva)가 칠 때는 방패 반대편이라 확률 ×scaeva — 다만 맞는 쪽도 왼손잡이면 서로 익숙하니 그대로. 치명타·허초는 그대로 방패를 넘는다
