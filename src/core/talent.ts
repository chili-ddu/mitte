...
export function awaken(rng: Rng, g: Gladiator, why: string): { from: Talent; to: Talent; why: string } | null { const t = talentOf(g); if (t >= AWAKEN_MAX || !rng.chance(AWAKEN_CHANCE)) return null; g.talent = (t + 1) as Talent; g.talentKnown = true; retalentCaps(g, t, g.talent); return { from: t, to: g.talent, why }; }
