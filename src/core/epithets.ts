...
export function accessoriesOf(g: Gladiator): EpithetAccessory[] { return Array.from(new Set((g.epithets ?? []).map(id => EPITHET_BY_ID[id as EpithetId]?.accessory).filter((a): a is EpithetAccessory => !!a))); }
