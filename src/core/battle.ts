...
export function battle(rng: Rng, teamA: Gladiator[], teamB: Gladiator[], opts: { hpBonusA?: number; boostedB?: Set<number>; boostMul?: number; traitsA?: TraitLevels; traitsB?: TraitLevels } = {}): BattleResult { // traitsA/B: 각 편성에서 센 특성 단계 (없으면 0 — 시뮬·테스트)
  ...
  const legendEv = (u: Unit, ids: string[]) => { if (u.g.legend && ids.includes(u.g.legend)) ev({ kind: 'dictata', actor: u.g.id, dictata: `L_${u.g.legend}` }); }; /* 전설의 순간 (2026-09-22 사용자: 고유 연출) — 화면이 이 이벤트로 그 인물의 연출을 튼다 */
  const follow = (a: Unit, b: Unit, when: AfterHit['when']): boolean => { for (const A of a.ms.afterHits) { if (A.when !== when || b.hp <= 0 || a.hp <= 0) continue; if (rng.chance(A.p)) { legendEv(a, when === 'feint' ? ['celadus'] : when === 'bound' ? ['crescens', 'prudens'] : when === 'charge' ? ['tetraites'] : []); if (strike(a, b, A.mult, masteryOf(a.g).find(m => m.eff.afterHit === A)?.id ?? 'follow', when === 'riposte' || when === 'deflect')) return true; } } return false; }; // 뒤따르는 타격: 조건이 맞는 것을 전부 굴린다 (쓰러뜨리면 true)
  const trip = (v: Unit, sec: number) => { if (v.ms.noTrip) { legendEv(v, ['priscus']); return false; } v.boundUntil = Math.max(v.boundUntil, t + sec * v.m.boundMul); v.holdUntil = Math.max(v.holdUntil, t + sec * v.m.boundMul); v.sprint = false; v.s.boundTimes++; return true; }; // 넘어뜨리기 (굳건히 서기는 무효)
  ...
  for (t = 0; t < MAX_T; t = +(t + DT).toFixed(2)) {
    ...
    for (const u of order) {
      ...
      if (u.ms.shieldWall && !u.wallUsed && u.hp < initialHp[u.g.id] * u.ms.shieldWall.at) { u.wallUsed = true; u.wallUntil = t + u.ms.shieldWall.sec; ev({ kind: 'dictata', actor: u.g.id, dictata: 'big_wall' }); legendEv(u, ['spiculus']); }
      if (u.ms.staminaIgnore && u.ignoreUntil < 0 && u.stamina < u.m.windedAt) { u.ignoreUntil = t + u.ms.staminaIgnore; ev({ kind: 'dictata', actor: u.g.id, dictata: 't_sec_breath' }); legendEv(u, ['flamma']); }
      ...
      if (u.ms.recharge && !u.sprint && d > u.reach && (t < target.boundUntil || t < target.openUntil) && t >= u.rechargeAt + 3) { u.sprint = true; u.rechargeAt = t; legendEv(u, ['tetraites']); } // 재돌격: 넘어지거나 묶인 상대에 다시 돌진
      ...
      for (let hit = 0; hit < (u.ms.comboTwice ? 3 : 2); hit++) {
        if (hit === 2) legendEv(u, ['hermes']); /* 세 번째 칼: 헤르메스의 순간 */
